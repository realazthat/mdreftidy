...
################################################################################
# Install mdreftidy and run smoke test
cd "${TMP_DIR}"
...
EXIT_CODE=0
python -m mdreftidy.cli --help || EXIT_CODE=$?
if [[ "${EXIT_CODE}" -eq 0 ]]; then
  echo -e "${RED}Expected mdreftidy to to fail in a clean environment${NC}"
  exit 1
fi
echo -e "${GREEN}Success: mdreftidy failed in a clean environment${NC}"

pip install "${DIST}"/*.whl
echo -e "${GREEN}Success: mdreftidy installed successfully${NC}"

python -m mdreftidy.cli --help
python -m mdreftidy.cli --version
